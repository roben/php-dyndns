...
$ipaddr = isset($argv[3]) ? $argv[3] : null;
$ip6addr = isset($argv[4]) ? $argv[4] : null;
...
$query = 'result/status';
$entries_status = $xpath->query($query);
if ($entries_status->length == 0) {
	trigger_error('no status in response', E_USER_ERROR);
	respond('failed', 'no status in response: ' . $status);
}

$status = $entries_status->item(0);
$error = false;
foreach ($status->childNodes as $child) {
	if ($child->nodeName == 'type' && $child->nodeValue == 'error') {
		$error = true;
	}
}
if ($error) {
	$msgs = array();
	foreach ($doc_result->getElementsByTagName('text') as $text) {
		$msgs[] = $text->nodeValue;
	}
	$msg = implode(PHP_EOL, $msgs);
	trigger_error('cannot get current zone records' . $msg, E_USER_ERROR);
	respond('failed', 'cannot get current zone records: ' . $msg);
}
...
if (null == $frag_data) {
	trigger_error('did not get zone, response: ' . $result, E_USER_ERROR);
	respond('failed', 'no zone');
}
...
if (isset($ipaddr)) {
	...
	$entries_status->item(0)->nodeValue = $ipaddr;
}
...
if (isset($ip6addr)) {
	...
	$entries_status->item(0)->nodeValue = $ip6addr;
}
...
$entries_status = $xpath->query($query);
if ($entries_status->length > 0) {
	$status = $entries_status->item(0)->nodeValue;
	if ($status != 'success') {
